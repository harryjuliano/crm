...
export default function Edit({ opportunityItem }) {
  const { data, setData, put, errors } = useForm({
        opportunity_id: opportunityItem.opportunity_id ?? '',
        item_type: opportunityItem.item_type ?? '',
        description: opportunityItem.description ?? '',
        qty: opportunityItem.qty ?? '',
        estimated_price: opportunityItem.estimated_price ?? '',
        estimated_discount_percent: opportunityItem.estimated_discount_percent ?? '',
        subtotal: opportunityItem.subtotal ?? '',
  })

  const submit = (e) => {
    e.preventDefault()
    put(route('apps.opportunity-items.update', opportunityItem.id))
  }

  return (
    <>
      <Head title='Edit Opportunity Item' />
      <Card title='Edit Opportunity Item' form={submit} footer={<Button type='submit' label='Update' variant='gray' />}>
